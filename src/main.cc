...
int main() {
    ...
    while (window.isOpen()) {
        ...
        if (!gameOver) {
            ...
            // player_instance.velocity = space ? 10 :
            ...
            if (count >= 10) {
                ...
                const int rounded_dist =
                    (650 - 25 + rand() % 50) / PIXEL_MULTIPLIER;

                pipes.push_back(new Pipe(rounded_x * PIXEL_MULTIPLIER,
                                         rounded_y * PIXEL_MULTIPLIER,
                                         rounded_dist * PIXEL_MULTIPLIER,
                                         pipeUpTexture, pipeDownTexture));
            }
            if (pipes[0]->getX() <= -200) {
                delete pipes[0];
