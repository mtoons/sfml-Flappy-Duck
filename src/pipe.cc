...
Pipe::Pipe(float x, float y, float distance, const sf::Texture *textureUp,
    ...
    if (IS_PIXEL) {
        ...
        pipeDown->setPosition(
            sf::Vector2<float>(x, (y) - (distance / 2) - 600));
...
void Pipe::move(float x) {
    ...
    if (IS_PIXEL) {
        ...
        pipeDown->setPosition(sf::Vector2<float>(rounded_x * PIXEL_MULTIPLIER,
                                                 pipeDown->getPosition().y));
        pipeUp->setPosition(sf::Vector2<float>(rounded_x * PIXEL_MULTIPLIER,
                                               pipeUp->getPosition().y));
    } else {
        pipeDown->setPosition(
            sf::Vector2<float>(posX, pipeDown->getPosition().y));
